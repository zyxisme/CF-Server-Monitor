<template>
  <a :href="'/server/' + server.id" class="server-card" :data-country="countryCode">
    ...
    <div class="server-meta">
      <div class="card-meta">
        <div v-if="sysConfig.show_price && server.price" class="card-meta-item"><span class="ms-icon">payments</span> {{ server.price }}</div>
        <div v-if="sysConfig.show_expire && server.expire_date" class="card-meta-item"><span class="ms-icon">calendar_month</span> <span :class="{ 'expired': isExpired }">{{ expireText }}</span></div>
      </div>
